	...
	if (_selection in _selections) then{
		...
		switch(true)do{
			...
			case (_projectile == "AW_R_M136_AT"):{
				if (_selection == "motor" || _selection == "NEtelo") then{
			...
			case (_projectile isKindOf "BulletBase"):{
				...
				switch(true)do{
					...
					case (_selection == "motor" || _selection == "NEtelo" || _selection == "telo"):{
						if (_newdamage > 0.0167) then{_newdamage = 0.0167;}; //cap new damage at 2% for hull and engine (T90 turret hit sometimes produces high engine damage)
					};
					case (_selection == "pas_P"):{
						_relDir = [_unit,getPos (_this select 3)]call _checkRelDir;
						if (_newdamage > 0.005 && (_relDir > 330 || _relDir < 210)) then{_newdamage = 0.05;}else{_newdamage = 0;};//set _newdamage to 0.05 if it's at least 0.005 damage by a single hit (needed because the T90s tracks seem to be very though to bullet fire compared to the T90s)
					};
			...
			case (_projectile == "AW_Sh_120_SABOT" || _projectile == "AW_Sh_125_SABOT"):{
				if ((_selection == "motor" || _selection == "NEtelo" || _selection == "telo" || _selection == "vez" || _selection == "zbran") && (_newdamage > 0.75)) then{
					_newdamage = 0.75;
				};
			};
			case (_projectile == "AW_Sh_120_HE" || _projectile == "AW_Sh_125_HE"):{
				if ((_selection == "motor" || _selection == "NEtelo" || _selection == "telo" || _selection == "vez" || _selection == "zbran") && (_newdamage > 0.4)) then{
					_newdamage = 0.4;
				};
			};
			///Arty
		...
		//----- DEBUG
		if (_selection == "motor" || _selection == "NEtelo" || _selection == "telo" || _selection == "pas_L" || _selection == "pas_P" || _selection == "vez" || _selection == "zbran") then{
		//if (_selection == "motor" || _selection == "NEtelo" || _selection == "vez" || _selection == "zbran") then{
		//if ((_selection == "pas_L" || _selection == "pas_P") && _newdamage > 0.005) then{
			if (isDedicated) then {
				PublicDebugMessage = format["# t: %5, Selection: %1, total:%2, new: %3, unmod.new: %4",_selection,[_damage]call _shortNum,[_newdamage]call _shortNum,[_uncapped]call _shortNum,diag_tickTime];
				PublicVariable "PublicDebugMessage";
			}
			else{
				PublicDebugMessage = format["# t: %5, Selection: %1, total:%2, new: %3, unmod.new: %4",_selection,[_damage]call _shortNum,[_newdamage]call _shortNum,[_uncapped]call _shortNum,diag_tickTime];
				PublicVariable "PublicDebugMessage";
				player sidechat format["# t: %5, Selection: %1, total:%2, new: %3, unmod.new: %4",_selection,[_damage]call _shortNum,[_newdamage]call _shortNum,[_uncapped]call _shortNum,diag_tickTime];
			};
		};
	}else{
		_damage = 0;
